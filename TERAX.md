...
- `git::commands::*`: full source-control surface (`git_status`, `git_diff`, `git_diff_content`, `git_stage`, `git_unstage`, `git_discard`, `git_commit`, `git_fetch`, `git_pull_ff_only`, `git_push`, `git_log`, `git_show_commit`, `git_commit_files`, `git_commit_file_diff`, `git_panel_snapshot`, `git_resolve_repo`, `git_remote_url`, `git_default_baseline`, `git_list_remote_branches`, `git_worktree_create`, `git_worktree_remove`, `git_review_status`). All gated through the workspace authorization registry. Worktree lifecycle: `git_worktree_create` makes `<repo sibling>/<repo>-<branch>` from a baseline ref (origin/HEAD, else main/master/develop), sets upstream for remote starts, applies `.worktreeinclude` globs as plain copies, and authorizes the new path; `git_worktree_remove` only deletes paths registered in `git worktree list` (never the main worktree) and can force-delete the branch. `git_diff`/`git_diff_content` take an optional `baseRef` for branch review: content anchors on `merge-base(baseRef, HEAD)` (the base commit itself when HEAD is unborn) and diffs to the working tree, so committed and uncommitted work both show.
